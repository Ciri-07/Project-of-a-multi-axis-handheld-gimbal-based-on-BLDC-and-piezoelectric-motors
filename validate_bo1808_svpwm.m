%% Validate the standalone BO1808 SVPWM Simulink model
...
simout_svpwm = simOut.get('simout_svpwm');

t = simout_svpwm.time;
y = simout_svpwm.signals.values;
...
% Phase voltage seen by a floating-neutral motor winding.
Va_pole = (Sa - 0.5)*Vdc;
Vb_pole = (Sb - 0.5)*Vdc;
Vc_pole = (Sc - 0.5)*Vdc;
V_common = (Va_pole + Vb_pole + Vc_pole)/3;
Van = Va_pole - V_common;
Vbn = Vb_pole - V_common;
Vcn = Vc_pole - V_common;

fprintf('\nBO1808 SVPWM validation\n');
fprintf('Vdc = %.4g V, fs_pwm = %.4g Hz, f_ref = %.4g Hz, Uref = %.4g V, m = %.4g\n', ...
    Vdc, fs_pwm, f_ref_svpwm, Uref_svpwm, m_svpwm);
fprintf('f_ref is derived from BO1808 rated speed: pn*Nrated/60 = %.4g Hz\n', f_ref_svpwm);
fprintf('Rated-point d-q voltage: ud = %.4g V, uq = %.4g V, |u_dq| = %.4g V\n', ...
    ud_ref_svpwm, uq_ref_svpwm, Uref_svpwm);
...
fig_ab = figure('Name','BO1808 SVPWM Alpha-Beta Reference','Color','w');
...
fig_ab_file = fullfile(figures_dir,'bo1808_svpwm_alpha_beta_ref.png');
...
fig = figure('Name','BO1808 Rated-Point SVPWM Validation','Color','w');
...
xlabel('Time / s')
ylabel('N')
title('(a) Sector N calculation result, display order 3-1-5-4-6-2')
ylim([0.5 6.5])
...
xlabel('Time / s')
ylabel('Tcm / s')
title('(b) Saddle-shaped modulation waves Tcm1/Tcm2/Tcm3')
...
fig_file = fullfile(figures_dir,'bo1808_svpwm_validation.png');
saveas(fig,fig_file);
fprintf('Saved plot: %s\n', fig_file);

fig_fft = figure('Name','BO1808 SVPWM A-Phase Voltage FFT','Color','w');
...
fig_fft_file = fullfile(figures_dir,'bo1808_svpwm_phase_voltage_fft.png');
