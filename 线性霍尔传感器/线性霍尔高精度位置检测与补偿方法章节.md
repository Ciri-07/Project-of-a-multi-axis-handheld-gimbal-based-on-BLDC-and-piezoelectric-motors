...
三篇文章分别代表三类思路：数据驱动自校准、复合信号提取与观测器补偿、传感与电机结构一体化设计。将三者结合后，可以形成适用于手持相机云台的“低成本线性霍尔传感器 + 高精度位置检测 + 误差补偿 + 闭环控制”的完整技术路线。
...
论文中采用三路线性 Hall 传感器，约相差 120 deg 电角度。初始位置可通过 Clarke 变换和 `atan2` 得到，随后利用闭环数据识别更准确的磁通密度函数。最终得到补偿函数后，Hall 测量误差可明显降低。论文实验中 RMS 测量误差约降低 2.6 倍。
...
```math
d_h(k) = g_h(\theta(k)) + v_h(k), \quad h=1,2,3
```
...
若三路 Hall 近似相差 120 deg 电角度，可先用 Clarke 变换得到初始正交量：

```math
\begin{bmatrix}
d_\alpha\\d_\beta\\d_0
\end{bmatrix}
=
\frac{2}{3}
\begin{bmatrix}
...
\operatorname{unwrap}
\left[
\operatorname{atan2}(d_\beta(k),d_\alpha(k))
\right]
```
...
1. 初始解角  
   采集多路 Hall 电压，进行零偏校正和幅值归一化，使用 Clarke 变换或正余弦变换得到初始角度。
...
2. 传感器布局  
   根据磁场分布决定 Hall 传感器数量、安装半径、夹角、气隙和方向。对旋转角检测，优先形成相差 90 deg 的正余弦信号。
...
- 每个旋转轴至少布置两路线性 Hall 信号，目标是形成正余弦关系。
- 若空间允许，可增加第三路 Hall 信号，提高冗余和自诊断能力。
